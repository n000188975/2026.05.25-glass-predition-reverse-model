...
st.set_page_config(page_title="玻璃預測與設計系統 v2.0", layout="centered")
...
# --- 逆向優化邏輯 (支援 5~15 種多元組分動態限制) ---
def inverse_predict(target_cte, target_young, target_viscosity, allowed_oxides, custom_bounds, max_elements, pkg):
    def loss_fn(weights, oxides_list):
        comp = {ox: w for ox, w in zip(oxides_list, weights)}
        preds = predict(comp, pkg)
        loss = 0.0
        
        if target_cte > 0: loss += (((preds.get("cte_1e-6_per_C", 0) - target_cte) / target_cte) ** 2) * 2.5
    ...
    for _ in range(500):
        ...
        for i, ox in enumerate(allowed_oxides):
            ...
            if ox == "SiO2": 
                ...
                guess.append(random.uniform(b_min, min(30, b_max)))
            else: 
                if random.random() > 0.4:
                    guess.append(random.uniform(b_min, min(8, b_max)))
                else:
                    guess.append(b_min + random.uniform(0, 0.2))
                    
        s = sum(guess)
        if s == 0: continue
        guess = [w / s * 100 for w in guess]
        
        if all(bounds[i][0] <= w <= bounds[i][1] for i, w in enumerate(guess)):
    ...
    res_initial = minimize(
        ...
        args=(allowed_oxides,), tol=1e-3, options={'maxiter': 60, 'eps': 0.4}
    )
    
    # 動態裁剪：依據使用者指定的 max_elements 限制保留組分數 (5~15)
    ...
    # 核心解鎖點：動態切片
    ox_weights = ox_weights[:max_elements]
    ...
    res_final = minimize(
        ...
        constraints={'type': 'eq', 'fun': lambda w: sum(w) - 100.0},
        args=(pruned_oxides,), tol=1e-4, options={'maxiter': 50, 'eps': 0.3}
    )
...
with tab2:
    ...
    t_cols = st.columns(3)
    with t_cols[0]:
        target_cte = st.number_input("目標 CTE", value=2.5, step=0.1, min_value=0.0)
    with t_cols[1]:
        target_young = st.number_input("目標 Young's Modulus (GPa)", value=80.0, step=1.0, min_value=0.0)
    with t_cols[2]:
        target_viscosity = st.number_input("目標 Viscosity (T10^3) (°C)", value=1430.0, step=10.0, min_value=0.0)
        
    st.subheader("2. 決定逆向配方搜尋範圍與架構")
    
    # 這裡新增控制項，解鎖 10~15 個元素的控制權
    max_elements = st.slider("✨ 允許生成的最大組分限制數量 (元素放寬上限)", min_value=5, max_value=15, value=12, step=1)
    
    search_mode = st.radio(
        "請選擇評估範圍：",
        [f"全元素自動篩選 (AI 將由資料庫中自動挑選最合適的常規與微量組分，上限 {max_elements} 種)", "自訂指定氧化物"],
        index=0
    )

    if search_mode.startswith("全元素自動篩選"):
    ...
    if st.button("🧩 生成實用配方", use_container_width=True):
        if len(inverse_selected) == 0:
            ...
            with st.spinner(f'AI 正在多維解空間進行優化 (當前允許最多 {max_elements} 個組分交互作用)...'):
                opt_comp, actual_res = inverse_predict(
                    target_cte, target_young, target_viscosity, 
                    inverse_selected, full_bounds, max_elements, model_pkg
                )
                
                st.success("✨ 配方生成完畢！")
                
                st.subheader(f"💡 實驗室推薦配方 (共 {len(opt_comp)} 種氧化物組分, Mass %)")
