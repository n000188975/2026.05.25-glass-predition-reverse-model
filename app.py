...
def inverse_predict(target_cte, target_young, target_viscosity, allowed_oxides, custom_bounds, pkg):
    def loss_fn(weights, oxides_list):
        ...
        # 物理目標誤差
        if target_cte > 0: loss += ((preds.get("cte_1e-6_per_C", 0) - target_cte) / target_cte) ** 2
        ...
        # 輕微的 L1 懲罰：如果一個元素對達標沒幫助，就逼它歸零，防止 AI 隨機分配 1.58%
        loss += 0.005 * sum(weights)
        return loss
    ...
    # 【核心修正】：賦予 AI 常識的初始值設定
    # 不要一開始大家平分，而是讓常見玻璃元素佔據主導地位
    init_guess = []
    for ox in allowed_oxides:
        if ox == "SiO2": 
            init_guess.append(60.0)
        elif ox in ["B2O3", "Al2O3", "Na2O", "CaO", "MgO", "K2O"]: 
            init_guess.append(6.0)
        else: 
            # 奇奇怪怪的元素一開始全部壓到趨近於 0
    ...
    init_guess = [w / s_mid * 100 for w in init_guess]
...
with tab2:
    ...
    if st.button("🧩 生成實用配方", use_container_width=True):
        if len(inverse_selected) == 0:
            ...
            with st.spinner('AI 正在調配配方 (啟動工程常識過濾與微量清理)...'):
