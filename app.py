...
def inverse_predict(target_cte, target_young, target_viscosity, allowed_oxides, custom_bounds, pkg):
    ...
    # 階段 1：大範圍初步探索
    init_guess = [(b[0] + b[1]) / 2.0 for b in bounds]
    s_mid = sum(init_guess)
    init_guess = [w / s_mid * 100 for w in init_guess] if s_mid > 0 else [100.0/len(bounds)]*len(bounds)

    res_initial = minimize(
        loss_fn, init_guess, method='SLSQP', bounds=bounds, constraints=constraints,
        args=(allowed_oxides,), tol=1e-2, options={'maxiter': 30}
    )
    
    # 階段 2：強制實用化修剪 (保留微量元素，但最多保留 Top 10)
    ox_weights = list(zip(allowed_oxides, res_initial.x))
    # 取消 0.5% 限制，只要比例大於 0.01% (有效微量) 就保留
    ...
    # 階段 3：使用精簡後的 Top 10 組合重新精確對準目標
    res_final = minimize(
        loss_fn, pruned_init, method='SLSQP', bounds=pruned_bounds, 
        constraints={'type': 'eq', 'fun': lambda w: sum(w) - 100.0},
        args=(pruned_oxides,), tol=1e-3, options={'maxiter': 40}
    )

    # 階段 4：實驗室級別的數據清理 (小數點 2 位，完美湊齊 100%)
    final_w = [round(w, 2) for w in res_final.x]
    s_f = sum(final_w)
    
    # 把因為四捨五入造成的微小誤差，補給佔比最大的那個元素
    if final_w:
...
with tab2:
    ...
    search_mode = st.radio(
        "請選擇評估範圍：",
        ["全元素自動篩選 (AI 將自動挑選最適合的 10 種以內成分)", "自訂指定氧化物"],
        index=0
    )

    if search_mode == "全元素自動篩選 (AI 將自動挑選最適合的 10 種以內成分)":
    ...
    if st.button("🧩 生成實用配方", use_container_width=True):
        if len(inverse_selected) == 0:
            ...
            with st.spinner('AI 正在調配實用級配方 (限制最多 10 種成分，保留關鍵微量元素)...'):
